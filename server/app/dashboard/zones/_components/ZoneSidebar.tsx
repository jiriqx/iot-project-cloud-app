    ...
    <aside className="w-52 shrink-0 border-r border-gray-200 bg-white overflow-y-auto">
      <div className="px-4 py-4">
        ...
        <ul className="space-y-0.5">
          {zones.map((zone) => {
            const isActive = pathname === `/zones/${zone.id}`
            return (
              <li key={zone.id}>
                <Link
        ...
        <Link
          ...
          className={`mt-3 flex items-center gap-1.5 px-3 py-2 rounded text-sm transition-colors ${pathname === '/dashboard/zones/create'
            ? 'bg-blue-50 text-blue-700 font-medium'
